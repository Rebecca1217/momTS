function factorData = sFactor_T1_004(data,para)
% ==================上行波动率因子================
% Type1:过去win天中找rtn>0
% Type2:找过去win个rtn>0
% --------------输入变量-------------------
...
% --------------处理过程------------------
% type1:数据缺失超过一半，记为nan；都是下行的收益，值为-inf
...
elseif type==2 %找过去win个rtn>0
    ...
    for c = 1:size(rtnData,2)
        tmp = rtnData(:,c);
        vltmp = nan(size(tmp));
        li = find(tmp>0); %正收益所在行
        if length(li)<win %正收益的个数达不到窗口期长度
            vltmp(:) = nan;
        else
