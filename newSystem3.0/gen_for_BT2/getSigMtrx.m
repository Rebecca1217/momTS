...
len = cellfun(@(x) size(x,1),TargetPortfolio(:,1));
num = sum(len);
t = 1;
targetTS = cell(num,2);
...
targetTS = sortrows([targetTS,num2cell(targetDate)],1);
fut_variety = regexp(targetTS(:,1),'\D*(?=\d)','match');
fut_variety = reshape([fut_variety{:}],size(fut_variety));
targetHD = [cell2mat(targetTS(:,3)),sign(cell2mat(targetTS(:,2))),abs(cell2mat(targetTS(:,2)))]; %date sign hands
%
futUni = unique(fut_variety);
date = cell2mat(TargetPortfolio(:,2));
signalMtrx = [date,zeros(length(date),length(futUni))];
HoldingMtrx = [date,zeros(size(signalMtrx))];
